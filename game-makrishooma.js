document.addEventListener("DOMContentLoaded", () => {
  ...
  const countriesOrder = ["iri", "china", "kazakhstan"];
  ...
  function getTargetList() {
    return Object.keys(gameData).filter(p => !gameData[p].eliminated);
  }
  ...
  function updateUI() {
    ...
    const alive = getTargetList();
    if (alive.length <= 1) {
      const winner = alive[0];
      document.getElementById("winnerText").textContent =
        winner ? `🏆 برنده: ${winner}` : "❌ بدون برنده";

      document.getElementById("gameStatus").textContent = "تمام شد";
    }
